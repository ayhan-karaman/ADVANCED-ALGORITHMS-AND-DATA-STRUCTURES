...
namespace DataStructures.Graphs.AdjacencySets
{
    public class DiGraph<T> : IDiGraph<T>
    {
        ...
        public IEnumerable<IDiGraphVertex<T>> VericesAsEnumerable => vertices.Select(x => x.Value);
        ...
        IEnumerable<IGraphVertex<T>> IGraph<T>.VericesAsEnumerable => vertices.Select(x => x.Value);
